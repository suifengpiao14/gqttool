...
select count(*) as `count` from `t_markdown`  where 1=1 {{template "PaginateWhere" "."}} ;
{{end}}

  {{define "Paginate"}}
select * from `t_markdown`  where 1=1 {{template "PaginateWhere" "."}}  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}
...
{{define "Del"}}
update `t_markdown` set `deleted_at`={{currentTime .}},`operator_id`=:OperatorID,`operator`=:Operator where `markdown_id`=:MarkdownID;
{{end}}
