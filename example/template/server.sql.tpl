...
{{define "Total"}}
select count(*) as `count` from `server`  where 1=1   and `deleted_at` is null;
{{end}}

 {{define "Paginate"}}
select * from `server`  where 1=1   and `deleted_at` is null  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}
