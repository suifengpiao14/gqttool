...
select count(*) as `count` from `t_validate_schema`  where 1=1 {{template "PaginateWhere" "."}} ;
{{end}}

  {{define "Paginate"}}
select * from `t_validate_schema`  where 1=1 {{template "PaginateWhere" "."}}  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}
...
{{define "Del"}}
update `t_validate_schema` set `deleted_at`={{currentTime .}},`operator_id`=:OperatorID,`operator`=:Operator where `validate_schema_id`=:ValidateSchemaID;
{{end}}
