...
{{define "Total"}}
select count(*) as `count` from `example`  where 1=1   and `deleted_at` is null;
{{end}}

 {{define "Paginate"}}
select * from `example`  where 1=1   and `deleted_at` is null  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}

{{define "Insert"}}
insert into `example` (`example_id`,`service_id`,`api_id`,`tag`,`title`,`summary`,`request`,`response`)values
