...
{{define "ddlParameter"}}
  ...
  `type` enum('string','int','number','array','object') NOT NULL DEFAULT 'string' COMMENT '参数类型',
  `tag` char(50) NOT NULL DEFAULT '' COMMENT '所属标签',
  `method` enum("get","post","put","delete","head")  NOT NULL DEFAULT 'POST' COMMENT '请求方法',
  ...
  `required` enum('true','false') NOT NULL DEFAULT 'false' COMMENT '是否必须(模型类为字符串)',
  `serialize` varchar(50) NOT NULL DEFAULT '' COMMENT '对数组、对象序列化方法,参照openapi parameters.style',
  `explode`  enum('true','false') NOT NULL DEFAULT 'true' COMMENT '对象的key,是否单独成参数方式,参照openapi parameters.explode',
...
{{define "ddlValidateSchema"}}
  ...
  `type` enum('integer','array','string','object') NOT NULL COMMENT '类型',
  `example` varchar(1024) NOT NULL DEFAULT '' COMMENT '案例',
  `deprecated` enum("true","false")  NOT NULL DEFAULT 'false' COMMENT '是否弃用',
  `required` enum('true','false') NOT NULL DEFAULT 'false' COMMENT '是否必须(模型类为字符串)',
  ...
  `read_only` enum('true','false') NOT NULL DEFAULT 'false' COMMENT '是否只读',
