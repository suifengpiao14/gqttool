...
select count(*) as `count` from `t_api`  where 1=1 {{template "PaginateWhere" "."}} ;
{{end}}

  {{define "Paginate"}}
select * from `t_api`  where 1=1 {{template "PaginateWhere" "."}}  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}
...
{{define "Del"}}
update `t_api` set `deleted_at`={{currentTime .}},`operator_id`=:OperatorID,`operator`=:Operator where `api_id`=:APIID;
{{end}}
