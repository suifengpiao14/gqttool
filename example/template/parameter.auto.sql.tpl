...
select count(*) as `count` from `t_parameter`  where 1=1 {{template "PaginateWhere" "."}} ;
{{end}}

  {{define "Paginate"}}
select * from `t_parameter`  where 1=1 {{template "PaginateWhere" "."}}  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}
...
{{define "Del"}}
update `t_parameter` set `deleted_at`={{currentTime .}},`operator_id`=:OperatorID,`operator`=:Operator where `parameter_id`=:ParameterID;
{{end}}
