...
{{define "Total"}}
select count(*) as `count` from `markdown`  where 1=1   and `deleted_at` is null;
{{end}}

 {{define "Paginate"}}
select * from `markdown`  where 1=1   and `deleted_at` is null  order by `updated_at` desc  limit :Offset,:Limit ;
{{end}}
